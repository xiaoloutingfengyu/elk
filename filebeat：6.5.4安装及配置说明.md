# filebeat：6.5.4安装及配置说明

linux

## [Step 1: Install Filebeat](https://www.elastic.co/guide/en/beats/filebeat/6.5/filebeat-installation.html) 
...
## [Step 2: Configure Filebeat](https://www.elastic.co/guide/en/beats/filebeat/6.5/filebeat-configuration.html) 

```
vim  filebeat-6.5.4-linux-x86_64/filebeat.yml
```
...
- type: log    #收集数据类型
  ...
  paths:             
    - /var/log/*.log      #目标数据地址
#    - /var/log/nginx/error.log     以此为例持续添加
output.elasticsearch:
  hosts: ["localhost:9200"]
#setup.kibana:
#  host: "mykibanahost:5601"    此配置是为了要为FileBeat设置Kibana仪表盘，如果Kibana与ElasticSearch在同一主机上运行，则可以跳过此步骤。      
```

## [Step 3: Set up the Kibana dashboards](https://www.elastic.co/guide/en/beats/filebeat/6.5/load-kibana-dashboards.html) 
...
## [Step 4: Start Filebeat](https://www.elastic.co/guide/en/beats/filebeat/6.5/filebeat-starting.html) 
...
filebeat输出到es的数据会以索引**filebeat-6.5.4-2019.03.01**形式存在
...
查找进程ID并kill掉：`ps` `-ef |``grep` `filebeat<br>``kill` `-9  进程号` 
